...
import { setFlights } from "@redux/features/flightSllice";
...
const ExploreScreen = () => {
	...
	const { useGetAirlineCities, useGetFlights } = useAirService();
	const {
		isLoading,
		data: airlineCities,
		isError,
	...
	useEffect(() => {
		if (isSuccessFlights) {
			console.log("flights in explore screen: ", flights);
		...
		// handle errors
		if (isErrorFlights) {
			console.log(errorFlights);
		}
	}, [isErrorFlights, isSuccessFlights, flights, errorFlights]);
